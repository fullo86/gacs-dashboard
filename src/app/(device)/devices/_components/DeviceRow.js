...
export default function DeviceRow({ device, visibleTags, toggleTags, onSelectDevice, showAlert }) {
  const actions = [
    ...
    { icon: LightningIcon, label: "Summon Device", onClick: async () => {
        try {
          ...
          showAlert("success", "Success", "Device Successfully Summoned");
        } catch (err) {
    ...
    { icon: AddRemoveTagIcon, label: "Add/Remove Tags", onClick: () => onSelectDevice(device, "tags", showAlert) },
